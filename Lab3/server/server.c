...
int main(int argc, char **argv){
    ...
    while(loop){
        ...
        //for testing the functionality of lab3
    ...
    while(1){
        if(recvfrom(sockfd, FIN_buf, BUF_SIZE, 0, (struct sockaddr*)&client_addr, &len) == -1){
            printf("recvfrom ERROR!\n");
        }
        string2packet(FIN_buf, &fin_packet);

        //check if FIN message received
        if(strcmp(fin_packet.filedata, "FIN") == 0){
            printf("Congratulations! File transfer success! YAYY!!\n");
            break;
        }
        else{
            ...
            char* ACK = "ACK";
            strcpy(fin_packet.filedata, ACK);

            //convert modified packet into string
            packet2string(FIN_buf, &fin_packet);

            //send string to client
            if((sendto(sockfd, FIN_buf, BUF_SIZE, 0, (struct sockaddr*)&client_addr, len) == -1)){
                printf("sendto ERROR!\n");
                exit(1);
            }
        }
